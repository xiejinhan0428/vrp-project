...
func SolveTspOneway(input TSPSolverInput) (output TSPSolverOutput) {
	...
	initDistance := 0.0
	initSeq := make([]int, len(input.DistanceMatrix))
	for i := 1; i < len(input.DistanceMatrix); i++ {
		initDistance += input.DistanceMatrix[i-1][i]
		initSeq[i] = i
	}
	//roundtrip模式
	if endIdx == startIdx {
		initDistance += input.DistanceMatrix[len(input.DistanceMatrix)-1][0]
		if iSSymmetrical {
		...
		if distance > initDistance {
			return TSPSolverOutput{initSeq, initDistance, retcode, err}
		}
