...
func genNodes(names []string, G [][]int, fixedEdges [][2]int) (nodes []*TSPNode, err error) {
	N := len(G)
...
func prim(nodes []*TSPNode, G [][]int) (err error) {
	N := len(nodes)
...
func min1Tree(nodes []*TSPNode, G [][]int) (cost int, err error) {
	if err = prim(nodes, G); err != nil {
		return 0, fmt.Errorf("min1Tree: failed to generate the MST: %w", err)
	}
	mstCost := 0

	var chosenLeafNode *TSPNode
	var secondNearestNeighbor *TSPNode
	...
	for _, n := range nodes {
		...
		if n.degree == 1 {
			...
			var secondNearestNeighborNow *TSPNode
...
func ascent(nodes []*TSPNode, G [][]int) (cost int, err error) {
...
func genCandidates(nodes []*TSPNode, G [][]int, maxCandidates int, maxNearness float64) (err error) {
