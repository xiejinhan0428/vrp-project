...
// TSPNode is a "city" in the travelling salesman problem.
type TSPNode struct {
	...
	pi int
	// The edge from this node to fixTSPNode1 must be in the final tour
	fixNode1 *TSPNode
	// The edge from node to fixTSPNode2 must be in the final tour
	fixNode2 *TSPNode
}

// isIn returns true if this node is an element of a given list of nodes.
func (n *TSPNode) isIn(nodeList []*TSPNode) bool {
...
type Tour struct {
	...
	size        int
	edges       map[Edge]bool
	idxMap      map[*TSPNode]int
	dis         float64
	symmetrical bool
}

// find nodes that nearby the input node
func (t *Tour) around(node *TSPNode) []*TSPNode {
	idx := t.idxMap[node]
	pred := idx - 1
	succ := idx + 1
	if succ == t.size {
	...
	return []*TSPNode{t.path[pred], t.path[succ]}
}

// init the edges from a path slice
func (t *Tour) basicInit() {
	idxMap := make(map[*TSPNode]int)
...
func (t *Tour) buildNewTour(destroy map[Edge]bool, repair map[Edge]bool) (bool, []*TSPNode) {
	...
	successors := make(map[*TSPNode]*TSPNode)
	node := t.path[0]
	edgeSilces := make([][]*TSPNode, t.size)
	...
	newPath := make([]*TSPNode, 0)
	//fmt.Println("\n*****")
	newPath = append(newPath, node)
	visited := make(map[*TSPNode]bool)
	visited[node] = true
